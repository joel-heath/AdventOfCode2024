...
public class Day05 : IDay
{
    ...
    public string SolvePart1(string input)
    {
        ...
        var orders = sections[0].Split(Environment.NewLine).Select(l => l.Split('|').Select(int.Parse).ToArray()).GroupBy(l => l[0]).ToDictionary(g => g.Key, g => g.Select(l => l[1]).ToHashSet());
        return $"{sections[1].Split(Environment.NewLine).Select(l => l.Split(',').Select(int.Parse).ToArray())
            .Sum(l => l.Select(
                    (curr, i) => l[..i].Any(
    ...
    {
        var sections = input.Split(Environment.NewLine + Environment.NewLine);
        var orders = sections[0].Split(Environment.NewLine).Select(l => l.Split('|').Select(long.Parse).ToArray()).GroupBy(l => l[0]).ToDictionary(g => g.Key, g => g.Select(l => l[1]).ToHashSet());
        return $"{sections[1].Split(Environment.NewLine).Select(l => l.Split(',').Select(long.Parse).ToArray()).ToArray()
